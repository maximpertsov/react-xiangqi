...
export const tryMarchMoves = (slot, moves, steps) => moves
  .reduce(
...
// FEN
export const fromFenRow = (row) => row.split('').reduce((acc, ch) => {
  const val = +ch;
  const newItems = Number.isNaN(val) ? [ch] : Array(val).fill(null);
  return acc.concat(newItems);
}, []);

export const fromFen = (fen) => fen.split('/').reduce(
  (acc, row) => acc.concat(fromFenRow(row)),
  [],
);
